...
void CTNR::Get_Params_from_File()
{
	...
	FileName.Format(_T("CNLJ_Naive_%s_%s_%d_%d.txt"), m_query_fname[0].GetBuffer(), m_data_fname[0].GetBuffer(), rand() % 1000, t.GetSecond() + t.GetMinute() + t.GetHour());
...
void CTNR::MkFN_search(CMySeq & qry_seq, const vector<pair<int, pair<float, float>>> & sorted_obj_seg_list)
{
	...
	{
		pair<int, float> & aa = qry_seq.qry_list.front();
		CQry& qry_aa = get_qry_from_tbl(aa.first);
		kFN_query(qry_seg, aa.second, qry_aa, sorted_obj_seg_list);

	}

	{
		pair<int, float> & zz = qry_seq.qry_list.back();
		CQry& qry_zz = get_qry_from_tbl(zz.first);
		kFN_query(qry_seg, zz.second, qry_zz, sorted_obj_seg_list);
